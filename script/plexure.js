...
(function (window) {
	...
	function createSample(sound, extra_info) {
		...
		var soundObj = {
			...
			preview_url: sound['preview-lq-mp3'],
			fixed: false
		};
		...
		if (extra_info) {
			...
			soundObj.wave_img.src = sound.waveform_l;
		}
			...
			onkeypress: function (e) {
				...
				if (e.target.localName === "textarea" || e.target.localName === "input") {
					...
					if (code === 13) {
						...
						freesound.search(query, 1, filter, 'created_desc', 6, null, 6, function (response) {
							for (var i = 1; i < response.sounds.length; i += 1) {
								freesound.get_sound(response.sounds[i].id, createSample, null);
							}
						}, null);
						e.target.value = "";
						return false;
